import { ModelInfo, ModelGroup, LocalModel, AIProvider, REMOTE_MODEL_PROVIDERS, LOCAL_MODEL_CONFIG, RemoteModelProvider, ModelItem, ModelState, ModelSelectValue } from '@/types/ai';
...
class ModelService {
    ...
    async fetchLocalModels(forceRefresh = false): Promise<LocalModel[]> {
        ...
        if (!forceRefresh && this.localModels.length > 0 && (now - this.lastFetchTime) < this.CACHE_DURATION) {
            console.log('Using cached local models:', this.localModels.length);
        ...
        try {
            ...
            console.log('Successfully fetched local models:', this.localModels.length, this.localModels);
            return this.localModels;
        } catch (error) {
            ...
            try {
                ...
                if (!isConnected) {
                    console.warn('Ollama service is not running or not accessible');
                }
            } catch (connError) {
                console.error('Failed to check Ollama connection:', connError);
            }
            return [];
        }
    }
    ...
    async getAllModels(): Promise<ModelGroup[]> {
        ...
        const remoteModelsByProvider = remoteModels.reduce((acc, model) => {
    ...
    hasRemoteApiConfigured(provider: RemoteModelProvider): boolean {
        ...
        console.log(`Checking API config for ${provider}:`, { config, isConfigured });
        return isConfigured;
    }
     ...
     */
    async getAllModelSelectValues(): Promise<{ value: ModelSelectValue, label: string, provider: AIProvider, type: 'local' | 'remote' }[]> {
        const groups = await this.getAllModels();
        const options: { value: ModelSelectValue, label: string, provider: AIProvider, type: 'local' | 'remote' }[] = [];
    ...
    getRemoteApiConfig(provider: RemoteModelProvider): { apiKey: string; baseURL: string } | null {
        try {
            ...
            console.log(`Getting API config for ${provider} from key ${storageKey}:`, config);
            return config;
        } catch (error) {
            console.error(`Failed to get API config for ${provider}:`, error);
            return null;
        }
    }
     ...
     */
    setRemoteApiConfig(provider: RemoteModelProvider, config: { apiKey: string; baseURL: string }): void {
        try {
            const storageKey = `api-config-${provider}`;
            localStorage.setItem(storageKey, JSON.stringify(config));
            console.log(`API config saved for ${provider} with key ${storageKey}:`, config);
        } catch (error) {
            console.error(`Failed to save API config for ${provider}:`, error);
        }
    }
    ...
    clearRemoteApiConfig(provider: RemoteModelProvider): void {
        try {
            ...
            console.log(`API config cleared for ${provider}`);
        } catch (error) {
            console.error(`Failed to clear API config for ${provider}:`, error);
        }
    }
}
