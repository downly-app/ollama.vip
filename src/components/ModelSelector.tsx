...
import { ChevronDown, ChevronRight, Globe, HardDrive, AlertCircle } from 'lucide-react';
import { ModelGroup, ModelInfo, AIProvider, RemoteModelProvider, ModelSelectValue, REMOTE_MODEL_PROVIDERS, LOCAL_MODEL_CONFIG } from '@/types/ai';
...
const ModelSelector = ({
    ...
    useEffect(() => {
        if (value && modelGroups.length > 0) {
            ...
            if (parsed) {
                console.log('Looking for model with parsed info:', parsed);

                const allModels = modelGroups.flatMap(group => group.models);
                console.log('Available models:', allModels.map(m => ({ provider: m.provider, id: m.id })));

                const model = allModels.find(m => {
                    const providerMatch = m.provider === parsed.provider;
                    const idMatch = m.id === parsed.model;
                    console.log(`Comparing model: ${m.provider}:${m.id} - Provider match: ${providerMatch}, ID match: ${idMatch}`);
                    return providerMatch && idMatch;
                });

                if (model) {
                    console.log('Found matching model:', model);
                    setSelectedModel(model);
                } else {
                    console.log('No matching model found for:', parsed);
    ...
    const handleModelSelect = async (model: ModelInfo, event: React.MouseEvent) => {
        ...
        if (model.type === 'local') {
            console.log('Local model selected, no API key check needed');
        ...
        if (model.type === 'remote' && model.requiresApiKey) {
            console.log('Remote model selected, checking API key configuration');
            const isConfigured = modelService.hasRemoteApiConfigured(model.provider as RemoteModelProvider);
            console.log('API key configured:', isConfigured);
            if (!isConfigured) {
                console.log('API key not configured, showing warning');
                setIsOpen(false);
                onShowApiKeyWarning?.();
                return;
            }
    ...
    return (
        ...
        `}
            ...
            {/* Dropdown menu */}
            {isOpen && (
                <div className="absolute top-full left-0 right-0 mt-1 bg-black/20 backdrop-blur-xl border border-white/30 rounded-xl shadow-2xl z-[9999] max-h-96 overflow-y-auto custom-scrollbar">
                    {isLoading ? (
                                                ...
                                                {group.models.map((model) => {
                                                    ...
                                                    return (
                                                        <button
                                                            ...
                                                            className={`w-full px-8 py-2 text-left hover:bg-black/10 transition-colors duration-200 border-0 bg-transparent cursor-pointer focus:outline-none focus:bg-black/15 ${isSelected ? 'bg-black/15' : ''
                                                                }`}
                                                        >
                                                            <div className="flex items-center justify-between">
                                                                <div className="flex items-center space-x-2 flex-1 min-w-0">
                                                                    <div className="flex-1 min-w-0">
                                                                        ...
                                                                        {model.parameters && (
                                                                            <div className="text-xs text-gray-500 truncate">{model.parameters}</div>
                                                                        )}
                                                                    </div>
                                                                </div>
                                                                <div className="flex items-center space-x-1">
