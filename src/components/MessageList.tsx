...
const MessageList = ({ onChatSelect, currentChatId, onShowApiKeyWarning }: MessageListProps) => {
  ...
  const handleCreateNewChat = async () => {
    try {
      ...
      const hasRemoteApi = modelService.hasRemoteApiConfigured('openai') || modelService.hasRemoteApiConfigured('deepseek');
  ...
  const handleMouseMove = useCallback((e: MouseEvent) => {
    ...
    if (newWidth >= 200 && newWidth <= 600) { // Limit min and max width
      setWidth(newWidth);
    }
  }, [isResizing]);
        ...
        {filteredChats.length === 0 ? (
          ...
          filteredChats.map((chat) => (
            <div
              ...
              className={`flex items-center px-4 py-4 cursor-pointer transition-all duration-200 border-b border-white/5 relative group ${currentChatId === chat.id
