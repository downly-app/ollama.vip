...
import { Loader2, Info, Settings, Code, Cpu, Database } from 'lucide-react';
...
const LocalModelInfoDialog: React.FC<LocalModelInfoDialogProps> = ({ modelName, open, onClose }) => {
    ...
    return (
      <div className="space-y-2">
        {params.map((param, index) => {
          ...
          return (
            <div key={index} className="flex justify-between items-center p-2 bg-white/5 backdrop-blur-sm rounded border border-white/10">
            ...
            {importantFields.map(field => {
              ...
              return (
                <div key={field} className="flex justify-between items-center">
                  <span className="text-white/70 text-sm">{field.replace('general.', '').replace('llama.', '')}</span>
  ...
  const renderCapabilities = () => {
    ...
    const getCapabilityColor = (capability: string) => {
      switch (capability.toLowerCase()) {
        case 'completion': return 'bg-green-500/20 text-green-300 border-green-500/30';
        case 'vision': return 'bg-blue-500/20 text-blue-300 border-blue-500/30';
        case 'embedding': return 'bg-purple-500/20 text-purple-300 border-purple-500/30';
  ...
  const renderContent = () => {
    if (isLoading) {
      return <div className="flex justify-center items-center h-[450px]"><Loader2 className="w-8 h-8 animate-spin text-white/70"/></div>;
    }
    ...
    return (
      <Tabs defaultValue="overview" className="w-full h-full">
        <TabsList className="grid w-full grid-cols-4 bg-white/5 backdrop-blur-sm border-white/10">
          ...
          <TabsTrigger value="architecture" className="data-[state=active]:bg-white/20 data-[state=active]:text-white text-white/70">
            <Cpu className="w-4 h-4 mr-2" />
            {t('localModels.dialog.tabs.architecture')}
          </TabsTrigger>
          <TabsTrigger value="parameters" className="data-[state=active]:bg-white/20 data-[state=active]:text-white text-white/70">
            <Settings className="w-4 h-4 mr-2" />
            {t('localModels.dialog.tabs.parameters')}
          </TabsTrigger>
          <TabsTrigger value="modelfile" className="data-[state=active]:bg-white/20 data-[state=active]:text-white text-white/70">
        ...
        <TabsContent value="overview" className="mt-4 h-[400px] overflow-y-auto pr-3">
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <Card className="bg-white/5 backdrop-blur-sm border-white/10">
                <CardHeader className="pb-3">
                  <CardTitle className="text-white/90 text-sm">{t('localModels.dialog.overview.basicInfo')}</CardTitle>
                </CardHeader>
                <CardContent className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span className="text-white/70">{t('localModels.dialog.overview.format')}</span>
                    <span className="text-white/90">{modelInfo.details.format}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-white/70">{t('localModels.dialog.overview.family')}</span>
                    <span className="text-white/90">{modelInfo.details.family}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-white/70">{t('localModels.dialog.overview.parameterSize')}</span>
                    <span className="text-white/90">{modelInfo.details.parameter_size}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-white/70">{t('localModels.dialog.overview.quantizationLevel')}</span>
                    <span className="text-white/90">{modelInfo.details.quantization_level}</span>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="bg-white/5 backdrop-blur-sm border-white/10">
                <CardHeader className="pb-3">
                  <CardTitle className="text-white/90 text-sm">{t('localModels.dialog.overview.supportedFamilies')}</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="flex flex-wrap gap-1">
                    {modelInfo.details.families.map((family, index) => (
                      <Badge key={index} variant="outline" className="bg-white/10 text-white/70 border-white/20 text-xs">
                        {family}
                      </Badge>
                    ))}
                  </div>
        ...
        <TabsContent value="modelfile" className="mt-4 h-[400px]">
          <div className="space-y-3 h-full">
            ...
            <ScrollArea className="bg-white/5 backdrop-blur-sm rounded-lg p-4 text-white/90 font-mono text-xs border border-white/10 custom-scrollbar" style={{height: modelInfo.template ? 'calc(50% - 2rem)' : 'calc(100% - 3rem)'}}>
            ...
            {modelInfo.template && (
              <>
                ...
                <h4 className="font-semibold text-white/90 text-sm">{t('localModels.dialog.configuration.templateConfig')}</h4>
                <ScrollArea className="bg-white/5 backdrop-blur-sm rounded-lg p-4 text-white/90 font-mono text-xs border border-white/10 custom-scrollbar" style={{height: 'calc(50% - 2rem)'}}>
