...
const SystemResourceMonitoring = () => {
  ...
  const generateSystemLogs = (info: any) => {
    ...
    const logs = [
      {
        timestamp: currentTime.toLocaleTimeString('zh-CN'),
        time: currentTime,
        level: 'info' as const,
    ...
    if (info.gpus.length > 0) {
      ...
      const gpuTime = new Date(Date.now() - 90000);
      logs.push({
        timestamp: gpuTime.toLocaleTimeString('zh-CN'),
        time: gpuTime,
    ...
    if (info.disks.length > 0) {
      ...
      const diskTime = new Date(Date.now() - 120000);
      logs.push({
        timestamp: diskTime.toLocaleTimeString('zh-CN'),
        time: diskTime,
      ...
      <Card className='bg-white/10 backdrop-blur-sm border-white/20'>
        ...
        <CardContent>
          <div className='text-center py-8'>
            ...
            <p className='text-red-400 mb-2'>{t('dashboard.systemResources.errors.fetchFailed')}</p>
  ...
  return (
    <>
                ...
                <div className='space-y-2'>
                  {recentLogs.length > 0 ? (
                    ...
                    <div className='text-center py-8 text-white/50'>
                      ...
                      <p>{t('dashboard.systemResources.recentLogs.noLogs')}</p>
                    </div>
                  )}
                </div>
              </div>
      ...
      {/* All System Logs Dialog */}
      <LogsDialog
        open={showAllLogs}
        onOpenChange={setShowAllLogs}
        title={t('dashboard.systemResources.recentLogs.allLogs')}
        description={t('dashboard.systemResources.recentLogs.allLogsDescription')}
        logs={recentLogs}
        onRefresh={fetchSystemResources}
        isLoading={isLoading}
        icon={FileText}
      />
    </>
