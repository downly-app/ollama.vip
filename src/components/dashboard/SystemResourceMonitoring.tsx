...
import { SystemInfo, SystemResourceData } from '@/types/system';
...
const SystemResourceMonitoring = () => {
  ...
  const [resources, setResources] = useState<SystemResourceData[]>([]);
  ...
  const fetchSystemResources = async () => {
    try {
      ...
      const info = await SystemApi.getSystemResources();
      setSystemInfo(info);
      
      // Log successful system resource retrieval activity
      activityLogger.addActivity('info', t('dashboard.systemResources.activities.resourcesUpdated'));
      
             // Convert to component required format
       const resourcesData: SystemResourceData[] = [
         {
           name: t('dashboard.systemResources.resources.cpu'),
         ...
         {
           name: t('dashboard.systemResources.resources.memory'),
         ...
         {
           name: t('dashboard.systemResources.resources.disk'),
           usage: info.disks.length > 0 ? info.disks[0].usage_percent : 0,
           color: '#f59e0b',
           icon: HardDrive,
           details: info.disks.length > 0 
             ? `${info.disks[0].used_gb.toFixed(1)}GB / ${info.disks[0].total_gb.toFixed(1)}GB`
             : 'N/A',
       ...
       if (info.gpus.length > 0) {
         ...
         if (gpu.core_usage_percent > 0) {
           ...
           gpuUsage = 0;
           gpuDetails = `${gpu.name} • ${SystemApi.formatGpuMemoryFromMB(gpu.memory_total_mb)} VRAM • ${t('dashboard.systemResources.usageUnavailable')}`;
         }
         
         resourcesData.push({
           name: t('dashboard.systemResources.resources.gpu'),
           usage: gpuUsage,
      ...
      generateSystemLogs(info);
      
    } catch (err) {
      console.error('Failed to fetch system resources:', err);
      const errorMessage = t('dashboard.systemResources.errors.fetchFailed');
      setError(errorMessage);
      
      // Log error activity
      activityLogger.addActivity('error', errorMessage);
      ...
      const fallbackResources: SystemResourceData[] = [
        {
          ...
          color: '#3b82f6',
          icon: Cpu,
          details: t('dashboard.systemResources.errors.unavailable'),
  ...
  // Generate system logs
  const generateSystemLogs = (info: SystemInfo) => {
    const logs = [
      {
        timestamp: new Date().toLocaleTimeString('zh-CN'),
        level: 'info' as const,
        message: t('dashboard.systemResources.refreshSuccess', { cpu: info.cpu.usage.toFixed(1), memory: info.memory.usage_percent.toFixed(1) })
      },
      {
  ...
  const formatValue = (resource: SystemResourceData) => {
    if (resource.name.includes('GPU') || resource.name.includes('gpu')) {
                ...
                <div key={index} className="space-y-4 p-4 bg-black/20 rounded-lg">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2">
                      <resource.icon size={20} className="text-white/70" />
            ...
            <div className="mt-6 pt-6 border-t border-white/10">
              ...
              <div className="space-y-2">
                {recentLogs.slice(0, 10).map((log, index) => (
                  <div key={index} className="bg-black/20 rounded p-3 text-sm hover:bg-black/30 transition-colors">
