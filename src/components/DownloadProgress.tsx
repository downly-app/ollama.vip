...
import { X, Pause, Play, RotateCw, Loader2 } from 'lucide-react';
                ...
                <div className="flex items-center justify-end mt-1">
                  {task.status === 'downloading' && (
                    <Button variant="ghost" size="sm" onClick={() => pauseDownload(task.id)}>
                      <Pause className="w-4 h-4 mr-1" />{t('downloads.pause')}
                    </Button>
                  )}
                  {task.status === 'paused' && (
                    <Button variant="ghost" size="sm" onClick={() => retryDownload(task.id)}>
                      <Play className="w-4 h-4 mr-1" />{t('downloads.resume')}
                    </Button>
                  )}
                  {task.status === 'error' && (
                    <Button variant="ghost" size="sm" onClick={() => retryDownload(task.id)}>
                      <RotateCw className="w-4 h-4 mr-1" />{t('downloads.retry')}
                    </Button>
                  )}
                </div>
              </>
