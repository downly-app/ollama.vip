      ...
      <div className="flex flex-col items-center space-y-3 px-2 flex-shrink-0">
        {toolbarItems.map((item, index) => (
          ...
          >
            ...
            {/* Hover tooltip */}
            <div className={`absolute left-14 bg-gradient-to-r ${currentTheme.colors.primary}/90 backdrop-blur-sm text-white text-xs px-3 py-1.5 rounded-md opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-nowrap z-[100] pointer-events-none shadow-lg border border-white/20`}>
              {item.label}
            </div>
          </div>
        ))}
