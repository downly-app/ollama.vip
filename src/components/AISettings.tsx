...
import { AIConfig, REMOTE_MODEL_PROVIDERS, RemoteModelProvider, AIProvider } from '@/types/ai';
import { modelService } from '@/services/modelService';
import { AlertCircle, Key, Globe, Settings as SettingsIcon, ExternalLink } from 'lucide-react';
...
const AISettings = ({ open, onClose, onSave, currentSettings }: AISettingsProps) => {
  ...
  const updateProviderConfig = (provider: RemoteModelProvider, field: 'apiKey' | 'baseURL', value: string) => {
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[480px] bg-white/10 backdrop-blur-xl border border-white/20 text-white shadow-2xl rounded-2xl">
        <DialogHeader>
          <DialogTitle className={`text-xl font-semibold bg-gradient-to-r ${currentTheme.colors.secondary} bg-clip-text text-transparent`}>
            {t('settings.title')}
          </DialogTitle>
        </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <div className="col-span-3 relative z-[9998]">
                <Select value={selectedProvider} onValueChange={(value) => setSelectedProvider(value as RemoteModelProvider)}>
                  <SelectTrigger className="bg-white/10 backdrop-blur-sm border-white/20 text-white hover:bg-white/15 focus:ring-0 focus:border-white/60 transition-all duration-200 rounded-xl">
                    <SelectValue placeholder={t('settings.selectProviderPlaceholder', 'Please select AI provider')} />
                  </SelectTrigger>
                  ...
                  <div className="flex items-start space-x-4">
                    ...
                    <div className="flex-1 space-y-2">
                      ...
                      <p className="text-sm text-white/70 leading-relaxed">
                        {t(`settings.providers.${selectedProvider}.description`, providerInfo.description)}
                      </p>
                  ...
                  <div className="space-y-4 pt-4 border-t border-white/10">
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        className="bg-white/10 backdrop-blur-sm border border-white/20 text-white placeholder:text-white/40 hover:bg-white/15 focus:bg-white/20 transition-all duration-200 rounded-lg"
                        placeholder={providerInfo.baseURL || t('settings.enterApiUrl', 'Enter API URL')}
                      />
