...
const Downloads = () => {
  ...
  const renderTaskActions = (task: DownloadTask) => {
    switch (task.status) {
      ...
      case 'pending':
        return (
          <Button size="icon" variant="ghost" onClick={() => pauseDownload(task.modelName)} className="hover:bg-white/10 text-white/70 hover:text-white">
            <Pause className="w-4 h-4" />
          </Button>
        );
      case 'paused':
        return (
          <Button size="icon" variant="ghost" onClick={() => retryDownload(task.modelName)} className="hover:bg-white/10 text-white/70 hover:text-white">
  ...
  const renderTaskList = (taskList: DownloadTask[], isCompletedList: boolean) => {
    if (taskList.length === 0) {
      return (
        <div className="flex items-center justify-center h-64 bg-black/10 rounded-lg">
          <p className="text-white/50">{t(isCompletedList ? 'downloads.noCompletedJobs' : 'downloads.noDownloadingJobs')}</p>
        </div>
      );
    }
    ...
    return (
      <div className="space-y-3">
        {taskList.map((task) => (
          <Card key={task.id} className="bg-white/5 border border-white/10 p-4 hover:bg-white/10 transition-colors duration-200">
            <div className="flex items-center justify-between gap-4">
              <div className="flex items-center gap-4 flex-1 min-w-0">
                <div className="shrink-0">
                  {isCompletedList ? <CheckCircle className="w-8 h-8 text-green-400" /> : <Database className="w-8 h-8 text-sky-400" />}
                </div>
              ...
              <div className="flex items-center gap-2 shrink-0">
                {isCompletedList && (
                   <Button size="icon" variant="ghost" onClick={() => setInfoModelName(task.modelName)} className="hover:bg-white/10 text-white/70 hover:text-white">
                      <Info className="w-4 h-4" />
                   </Button>
                )}
                {renderTaskActions(task)}
                <Button size="icon" variant="ghost" onClick={() => clearTask(task.modelName)} className="hover:bg-white/10 text-white/70 hover:text-white">
            ...
            {!isCompletedList && task.status !== 'pending' && (
              <div className="mt-3">
                <Progress value={task.progress} className="w-full h-2 bg-black/20 [&>div]:bg-purple-500" />
              </div>
            )}
          </Card>
        ))}
          ...
          <Tabs defaultValue="downloading" className="h-full flex flex-col">
            <div className="flex justify-between items-center mb-4">
              <TabsList className="grid w-full grid-cols-2 bg-transparent border border-white/10 p-1 h-auto self-start max-w-sm">
                <TabsTrigger value="downloading" className="data-[state=active]:bg-white/10 data-[state=active]:text-white text-white/70 rounded-md">
                  {t('downloads.downloading')} ({downloadingTasks.length})
                </TabsTrigger>
                <TabsTrigger value="completed" className="data-[state=active]:bg-white/10 data-[state=active]:text-white text-white/70 rounded-md">
                   {t('downloads.completed')} ({completedTasks.length})
                </TabsTrigger>
              </TabsList>
              {completedTasks.length > 0 && (
                <Button variant="ghost" onClick={clearAllCompleted} className="text-white/60 hover:text-white hover:bg-white/10">
